...
class JecVariations(object):
    ...
    def do_jer_correction(self, event):
        ...
        jersfs = np.ones_like(event.Jet.pt.content, dtype=np.float32)
        jersfs_up = np.ones_like(event.Jet.pt.content, dtype=np.float32)
        jersfs_down = np.ones_like(event.Jet.pt.content, dtype=np.float32)
