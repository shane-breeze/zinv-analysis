...
def evaluate_xclean_mask(ev, source, nsig, obj1name, obj2name, mindr):
    ...
    return awk.JaggedArray(
        obj1.eta.starts, obj1.eta.stops, mask,
    )
...
class ObjectCrossCleaning(object):
    ...
    def begin(self, event):
        for cname in self.collections:
            for rname in self.ref_collections:
                event.register_function(
                    event,
                    "{}_XCleanMask{}".format(cname, rname),
                    partial(
                        evaluate_xclean_mask, obj1name=cname,
                        obj2name=rname, mindr=self.mindr,
                    ),
                )
